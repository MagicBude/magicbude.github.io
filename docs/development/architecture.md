...
页面结构、样式、脚本、字体、图片和图标继续由仓库本地托管，不通过 CDN 引入第三方运行时代码。确有实时性价值的数据可以由浏览器直接请求经过评估的外部 API，但必须满足以下条件：

- 外部请求失败时保留清晰提示，不能阻断导航、阅读或站内搜索。
- 涉及位置等隐私数据时，必须由访客主动操作触发；只发送实现功能所需的最低精度数据，不在本站持久化。
- 不能把 API 密钥写入浏览器代码；需要密钥时改用服务端或 serverless 代理。
- 在界面和文档中遵守数据来源的署名、许可、用途和调用限额。

首页天气使用 Open-Meteo 免费非商业 API。访客点击后才申请浏览器定位，坐标约简到两位小数再请求当前天气，页面刷新后不保留位置。若站点以后加入广告、订阅或其他商业用途，必须先迁移到商业接口或自托管方案。
